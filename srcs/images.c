...
/*   Updated: 2024/03/25 13:12:15 by vkettune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	place_tile(mlx_t *mlx, t_map *map, t_grid *pos, t_images *images)
{
	if (pos->tile == '1')
	{
		pos->tile_inst = mlx_image_to_window(mlx, images->wall_img, pos->x * map->tile_size, pos->y * map->tile_size);
		pos->tile_img = images->wall_img;
	}
	else
	{
		pos->tile_inst = mlx_image_to_window(mlx, images->floor_img, pos->x * map->tile_size, pos->y * map->tile_size);
...
int	place_object(mlx_t *mlx, t_map *map, t_grid *pos, t_images *images)
{
	...
	if (pos->tile == 'E')
	{
		pos->obj_inst = mlx_image_to_window(mlx, images->exit_img, pos->x * map->tile_size, pos->y * map->tile_size);
		pos->obj_img = images->exit_img;
	}
	else if (pos->tile == 'C')
	{
		pos->obj_inst = mlx_image_to_window(mlx, images->coin_img, pos->x * map->tile_size, pos->y * map->tile_size);
...
int	place_player(mlx_t *mlx, t_map *map, t_grid *pos, t_images *images)
{
	if (pos->tile != 'P')
		return (1);
	map->player.inst = mlx_image_to_window(mlx, images->player_img, pos->x * map->tile_size, pos->y * map->tile_size);
